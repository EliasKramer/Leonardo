...
namespace AiTest
{
	...
	public:
		TEST_METHOD(fools_mate_white)
		{
			...
			matrix w_value(leonardo_util::get_value_nnet_output());
			matrix b_value(leonardo_util::get_value_nnet_output());

			leonardo_util::set_value_nnet_output(w_value, board, White);
			leonardo_util::set_value_nnet_output(b_value, board, Black);

			matrix win_matrix(leonardo_util::get_value_nnet_output());
			win_matrix.set_at_flat_host(0, 1);
			win_matrix.set_at_flat_host(1, 0);
			matrix lose_matrix(leonardo_util::get_value_nnet_output());
			...
			Assert::AreEqual(-1.0f, leonardo_util::get_value_nnet_output(w_value));
			Assert::AreEqual(1.0f, leonardo_util::get_value_nnet_output(b_value));
		}
