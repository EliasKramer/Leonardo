...
def search(game, policy_nn, prediction_nn):
    if gameOver(input_matrix): return outcome(input_matrix)

    if game.input_matrix() not in visited:
        visited.append(game.input_matrix())
        P[game.input_matrix()][all_moves], evaluation = prediction_nn.predict(game.input_matrix())
        return -evaluation
    ...
    for curr_move in legalMoves(game):
        ...
        u = Q[game.input_matrix()][curr_move] + c * \
    ...
    new_game = makeMove(game, curr_move)
    # we make the best move with e&e in mind and then search again
    # we do this recursively until we reach a leaf node
    evaluation = search(new_game, policy_nn, prediction_nn)

    Q[game.input_matrix()][curr_move] = \
    ...
    global N[input_matrix][curr_move]
    N[input_matrix][curr_move] += 1
    return -evaluation
...
def policy(game, policy_network, prediction_network):
    ...
    #number_of_simulations = 25000 on alphazero
